...
def segmentation_keras_load1():
    ...
    model = models.att_unet_2d((256,256, 3), filter_num=[64, 128, 256, 512, 1024], n_labels=1, 
                           ...
                           name='attunet')
    model_fig_file = 'Output/Modelfigure/segmentation_model_u_net.png'
    tf.keras.utils.plot_model(model, to_file=model_fig_file, show_shapes=True)
    model.compile(optimizer="adam", loss="binary_crossentropy", metrics=METRICS)
    checkpoint = tf.keras.callbacks.ModelCheckpoint("FireSegmentationatt.h5", save_best_only=True)
    ...
    tf.keras.callbacks.ModelCheckpoint("FireSegmentationatt.h5", save_best_only=True),
    ...
    model_predict = tf.keras.models.load_model("FireSegmentation.h5")
